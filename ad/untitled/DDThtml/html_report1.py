...
class HtmlReport():
    ...
    def write_data_to_db(self,version,casetype,module,caseid,casename,result,error,screen,runtime1):
        # runtime = self.get_now_time()
        # print(runtime)
        sql = 'insert into htmlreport.html_report VALUES (default,"%s","%s","%s","%s","%s","%s","%s","%s",%s)'%(version,casetype,module,caseid,casename,result,error,screen,runtime1)
    ...
    def generate_report(self,version):
        ...
        if len(all_data)>0:
            ...
            with open("./htmlreport1/js_html/html2.html","r",encoding="utf8") as f:
                content = f.read()
                #执行时间
            ...
            for data in all_data:
                ...
                if  str(data[6]) == "成功":
                    ...
                    case_content = case_content+'<td width="10%">'+"<a href = ./js_html/1.png>"+"</a>"+"点这里"+'</td>\n'
            ...
            with open("./htmlreport1/js_html/html3.html","w",encoding="utf8") as f:
